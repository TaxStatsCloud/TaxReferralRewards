...
export async function sendReferralInvitation(email: string, referralCode: string, referrerName: string): Promise<boolean> {
  ...
  return sendEmail({
    to: email,
    subject: `${referrerName} invited you to join TaxStats`,
    text: `Hello,\n\n${referrerName} has invited you to join TaxStats. Use this referral link to sign up: ${referralLink}\n\nThanks,\nTaxStats Team`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <div style="background-color: #3B82F6; padding: 20px; text-align: center; color: white;">
          <h2>TaxStats Referral Invitation</h2>
        </div>
        <div style="padding: 20px; border: 1px solid #e5e7eb; border-top: none;">
          <p>Hello,</p>
          <p><strong>${referrerName}</strong> has invited you to join TaxStats!</p>
